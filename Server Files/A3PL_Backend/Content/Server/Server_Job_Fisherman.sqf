...
["Server_JobFisherman_DeployNet",
{
	...
	_buoy setVariable ["owner",(_player getVariable ["character_id",""]),true];
	...
	[getPlayerUID _player,(_player getVariable ["character_id",""]),"Fishing_NetDeploy",[format ["Pos: %1",(getPosATL _buoy)]]] call Server_Log_New;
}] call compile_Server;
...
["Server_JobFisherman_GrabNet",
{
	...
	// Check if player has the fisher trait
	private _traits = _player getVariable ["Player_Traits", []];
	private _hasFisherTrait = "fisher" in _traits;
	private _luckyBonus = [_traits] call A3PL_Traits_GetLuckyBonus;
	private _yieldMultiplier = 1;

	// Fisher trait: 50% chance to double the yield (+ lucky bonus, or lucky alone: 15%)
	private _chance = if (_hasFisherTrait) then {50 + _luckyBonus} else {_luckyBonus};
	...
	switch (true) do {
		case ((_player inArea "A3PL_Marker_Fish1") OR (_player inArea "A3PL_Marker_Fish2") OR (_player inArea "A3PL_Marker_Fish4")): {
			[_player,"bucket_full",1 * _yieldMultiplier] call Server_Inventory_Add;
			[_player,"mullet",1 * _yieldMultiplier] call Server_Inventory_Add;
		...
		case ((_buoy getVariable ["bait","none"]) == "shark"): {
			...
			switch (true) do {
				case (_random >= 65): {[_player,"shark_2lb",1 * _yieldMultiplier] call Server_Inventory_Add;};
				case (_random >= 40): {[_player,"shark_4lb",1 * _yieldMultiplier] call Server_Inventory_Add;};
				case (_random >= 25): {[_player,"shark_5lb",1 * _yieldMultiplier] call Server_Inventory_Add;};
				case (_random >= 10): {[_player,"shark_7lb",1 * _yieldMultiplier] call Server_Inventory_Add;};
				default {[_player,"shark_10lb",1 * _yieldMultiplier] call Server_Inventory_Add;};
			};
	...
	[getPlayerUID _player,(_player getVariable ["character_id",""]),"Fishing_NetCollect",[format ["Pos: %1",(getPosATL _buoy)]]] call Server_Log_New;
}] call compile_Server;
