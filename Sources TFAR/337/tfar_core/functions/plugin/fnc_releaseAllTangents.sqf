...
if !(_unit getVariable ["A3PL_CurrentNameTFAR",""] isEqualTo "") then
{
    _name = _unit getVariable ["A3PL_CurrentNameTFAR",""];
};

"task_force_radio_pipe" callExtension (format ["RELEASE_ALL_TANGENTS	%1~", _unit getVariable ["TFAR_unitName", _name]]); //Async call will always return "OK"
